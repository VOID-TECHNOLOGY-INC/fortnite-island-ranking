...
app.get(['/islands/:code/research', '/api/islands/:code/research'], async (req, res) => {
  ...
  try {
    ...
    const userPrompt = lang === 'ja'
      ? `Fortniteの島「${titlePart}」について、いまの盛り上がり状況、概要、特徴、話題点を箇条書き中心で簡潔に要約してください。誤検出を避けるため、島コードに合致しない情報は除外してください。最後に参考リンクを数件列挙してください。`
      : `Research the Fortnite island "${titlePart}". Summarize current buzz/activity, overview, key features, and notable discussions in concise bullet points. Avoid mismatches; ignore info not matching the island code. Include a few reference links at the end.`;
    ...
    for (const model of candidates) {
      const body = {
        model,
        messages: [
          { role: 'system', content: 'You are a concise research assistant for Fortnite islands. Provide factual, link-cited summaries.' },
